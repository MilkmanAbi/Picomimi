...
#define MAX_APP_MEM_REQUEST_GLOBAL (80 * 1024)
...
kout.println(" app block list - List blocked apps");
kout.println(" app block unlock <name> - [ROOT] Unblocks an app");
kout.println("\n=== Applications ===");
...
kout.print("\nAffinity: "); kout.println(aff_str[task_copy.affinity]);
const char* state_str[] = {"READY", "RUNNING", "WAITING", "SUSPENDED", "TERMINATED", "ZOMBIE"};
kout.print("\nState: "); kout.println(task_copy.mem_blocked ? "BLOCKED" : state_str[task_copy.state]);
const char* type_str[] = {"KERNEL", "DRIVER", "SERVICE", "MODULE", "APPLICATION"};
uint8_t type_idx = 0;
if (task_copy.task_type == TASK_TYPE_DRIVER) type_idx = 1;
else if (task_copy.task_type == TASK_TYPE_SERVICE) type_idx = 2;
else if (task_copy.task_type == TASK_TYPE_MODULE) type_idx = 3;
else if (task_copy.task_type == TASK_TYPE_APPLICATION) type_idx = 4;
kout.print("Type: "); kout.println(type_str[type_idx]);
kout.print("Base Priority: "); kout.println(task_copy.sched_info.base_priority);
kout.print("Eff. Priority: "); kout.println(task_copy.priority);
if (task_copy.original_priority != 0 && task_copy.original_priority != task_copy.priority) {
kout.print("Orig. Priority: "); kout.println(task_copy.original_priority);
kout.println(" (Boosted by Priority Inheritance)");
}
kout.print("OOM Priority: "); kout.println(task_copy.oom_priority);
kout.print("\nMemory Used: ");
...
kout.println(" KB");
if (task_copy.mem_request_bytes > 0) {
kout.print("Memory Request: ");
kout.print(task_copy.mem_request_bytes / 1024);
kout.println(" KB");
}
if (task_copy.mem_limit > 0) {
kout.print("Memory Limit: ");
kout.print(task_copy.mem_limit / 1024);
kout.println(" KB");
} else {
...
kout.println(task_copy.page_faults);
kout.print("Alloc Velocity: ");
kout.print(task_copy.alloc_velocity);
kout.println(" allocs/sec");
uint64_t uptime = (get_time_ms() - task_copy.start_time) / 1000;
kout.print("\nUptime: ");
kout.print((uint32_t)uptime);
kout.println(" s");
if (task_copy.max_runtime > 0) {
kout.print("Max Runtime: ");
kout.print((uint32_t)(task_copy.max_runtime / 1000));
kout.println(" s");
}
if (task_copy.respawn_count > 0) {
kout.print("Respawn Count: ");
kout.println(task_copy.respawn_count);
}
kout.println("\n--- Flags ---");
if (task_copy.flags & TASK_FLAG_PROTECTED) kout.println(" PROTECTED");
if (task_copy.flags & TASK_FLAG_CRITICAL) kout.println(" CRITICAL");
if (task_copy.flags & TASK_FLAG_RESPAWN) kout.println(" RESPAWN");
if (task_copy.flags & TASK_FLAG_ONESHOT) kout.println(" ONESHOT");
if (task_copy.flags & TASK_FLAG_PERSISTENT) kout.println(" PERSISTENT");
if (task_copy.flags & TASK_FLAG_OOM_CLEANUP_REQUESTED) kout.println(" OOM_CLEANUP_REQUESTED");
if (task_copy.mem_blocked) kout.println(" MEM_BLOCKED");
kout.println("\n--- IPC Queue ---");
kout.print(" Messages pending: "); kout.println(task_copy.ipc.message_count);
kout.print(" Priority Bitmap: 0x"); kout.println(task_copy.ipc.priority_bitmap, HEX);
...
TCB* task = NULL;
bool is_core1 = (oom_handlers[i].task_id >= 1000);
if (is_core1) mutex_enter_blocking(&kernel.core1.scheduler_lock);
task = ipc_get_tcb_by_id_unsafe(oom_handlers[i].task_id);
if (task) {
kout.print(" - ");
...
kout.print((HEAP_SIZE - KERNEL_RESERVE) / 1024);
kout.println(" KB");
kout.print("Used (Total): ");
kout.print(get_used_memory() / 1024);
kout.println(" KB");
kout.print("Free (Total): ");
...
kout.print("App not found in block list: ");
kout.println(arg);
}
}
void setup() {
...
randomSeed(micros());
kout.println("========================================");
kout.println(" Picomimi Kernel v11 A1");
kout.println(" (Hardened RTOS)");
kout.println("========================================");
...
OOM_PRIORITY_NEVER, 0, 0, NULL, "Kernel idle task", CORE_0);
kout.println("[OK] Idle (Pri 0, Core 0)");
task_create("k_reaper", k_reaper_task, NULL, 1,
TASK_TYPE_KERNEL, TASK_FLAG_PROTECTED | TASK_FLAG_CRITICAL | TASK_FLAG_RESPAWN, 0,
OOM_PRIORITY_NEVER, 1 * 1024, 1 * 1024, NULL, "Zombie task reaper", CORE_0);
...
OOM_PRIORITY_NEVER, 1 * 1024, 1 * 1024, &input_callbacks, "Focus cycle driver", CORE_0);
...
OOM_PRIORITY_NORMAL, 4 * 1024, 4 * 1024, &shell_callbacks, "Command shell", CORE_0);
kout.println("[OK] Shell service (Pri 10, Core 0)");
task_create("cpumon", NULL, NULL, 2,
TASK_TYPE_SERVICE, TASK_FLAG_PROTECTED | TASK_FLAG_RESPAWN, 0,
OOM_PRIORITY_NEVER, 2 * 1024, 2 * 1024, &cpumon_callbacks, "CPU monitor", CORE_0);
kout.println("[OK] CPU monitor (Pri 2, Core 0)");
task_create("tempmon", NULL, NULL, 2,
TASK_TYPE_SERVICE, TASK_FLAG_PROTECTED | TASK_FLAG_RESPAWN, 0,
OOM_PRIORITY_NEVER, 2 * 1024, 2 * 1024, &tempmon_callbacks, "Temp monitor", CORE_0);
kout.println("[OK] Temp monitor (Pri 2, Core 0)");
if (kernel.fs_available) {
task_create("fs", NULL, NULL, 8,
TASK_TYPE_SERVICE, TASK_FLAG_PROTECTED | TASK_FLAG_RESPAWN, 0,
OOM_PRIORITY_NEVER, 4 * 1024, 4 * 1024, &fs_callbacks, "FS service", CORE_0);
kout.println("[OK] FS service (Pri 8, Core 0)");
}
kout.println("========================================");
kout.println("Kernel boot complete!");
kout.println("========================================");
kout.print("Heap: "); kout.print((HEAP_SIZE - KERNEL_RESERVE) / 1024); kout.print(" KB (App) + ");
kout.print(KERNEL_RESERVE / 1024); kout.println(" KB (Sys)");
...
klog(0, "KERNEL: Boot v11 A1 (Hardened)");
shell_prompt();
kernel.running = true;
}
void loop() {
...
for(uint32_t i=0; i < kernel.task_count; i++) {
if (kernel.tasks[i].id != sender_id && kernel.tasks[i].state != TASK_ZOMBIE && !kernel.tasks[i].mem_blocked) {
...
if (task == NULL) {
if(is_core1) mutex_exit(&kernel.core1.scheduler_lock); else enable_all_interrupts();
return false;
}
...
TCB* task = &kernel.tasks[i];
if (task->state == TASK_TERMINATED || task->state == TASK_ZOMBIE || task->mem_blocked) continue;
...
if (allocator_task->alloc_velocity > OOM_ABUSIVE_ALLOC_VELOCITY && allocator_task->task_type == TASK_TYPE_APPLICATION) {
kout.println("\n[OOM] ABUSIVE ALLOCATOR DETECTED!");
kout.print("[OOM] Executing allocator '"); kout.print(allocator_task->name); kout.println("'");
...
kout.println(")");
if (victim_task && victim_task->alloc_velocity > OOM_ABUSIVE_ALLOC_VELOCITY) {
...
if ((task->mem_limit > 0 && current_usage + size > task->mem_limit) || (task->mem_request_bytes > 0 && current_usage + size > task->mem_request_bytes)) {
char klog_buf[128];
snprintf(klog_buf, sizeof(klog_buf), "MEM_PROTECT: Task %s (ID %d) exceeded memory limit. Killing.", task->name, task_id);
klog(3, klog_buf);
mutex_exit(&kernel.mem_lock);
...
} else if (current_usage > task->mem_throttle_mark && current_usage - task->mem_throttle_mark > VELOCITY_CHECK_CHUNK) {
uint64_t delta_time = now_us - task->mem_throttle_time_us;
if (delta_time < VELOCITY_TIME_THRESHOLD_US && delta_time > 0) {
char klog_buf[128];
snprintf(klog_buf, sizeof(klog_buf), "MEM_PROTECT: Task %s (ID %d) mem velocity too high! Throttling.", task->name, task_id);
klog(1, klog_buf);
mutex_exit(&kernel.mem_lock);
task_sleep(250);
mutex_enter_blocking(&kernel.mem_lock);
...
if (now_alloc - task->last_alloc_time > 1000) { task->alloc_velocity = 0; }
task->alloc_velocity++;
...
char klog_buf[64];
snprintf(klog_buf, sizeof(klog_buf), "MEM_PROTECT: Launch rejected, app %s is blocked.", name);
klog(2, klog_buf);
return 0;
}
}
...
char klog_buf[128];
snprintf(klog_buf, sizeof(klog_buf), "MEM_PROTECT: App %s launch rejected, requested %luKB > max %luKB.", name, mem_request / 1024, (uint32_t)MAX_APP_MEM_REQUEST_GLOBAL / 1024);
klog(3, klog_buf);
...
for (uint32_t i = 0; i < kernel.task_count; i++) {
if (kernel.tasks[i].alloc_velocity > 0 && (now_ms_for_velocity - kernel.tasks[i].last_alloc_time > 1000)) {
...
void brutal_task_kill(uint32_t id) {
if (id >= 1000) {
kout.print("[KILL] Core 1 task "); kout.println(id);
mutex_enter_blocking(&kernel.core1.scheduler_lock);
...
kout.println(" > [OMNIMAN] Stripping RESPAWN flag.");
