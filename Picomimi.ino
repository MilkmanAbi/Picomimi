//Picomimi v9.5 - Task Termination Fix
//CHANGES FROM v9.4:
//- FIX: Terminated GUI applications now correctly transition to the `TASK_TERMINATED` state instead of re-entering the scheduler as `TASK_READY`.
//- FIX: This resolves an issue where an exited app could prevent new apps from launching or cause UI conflicts by not being properly killed.
//- FIX: GUI applications (e.g., Snake) now call `brutal_task_kill` upon exit, ensuring a full deinitialization process.
//- FIX: This correctly releases GUI focus, calls the `deinit` callback, and frees all associated memory, preventing "ghost" screens when returning to the terminal.
//- FIX: Switching from a GUI application back to the terminal now correctly revokes the application's GUI focus, preventing it from continuing to draw over the terminal.
//- FIX: Corrected a critical context bug where task `init` functions were running as the calling task (e.g., 'shell').
//- REFACTOR: The `init` callback in `ModuleCallbacks` now receives the new task's ID, ensuring correct ownership for memory allocation and GUI focus requests.
...
void shell_execute(char* cmd) {
    ...
    if (strcmp(cmd, "help") == 0) cmd_help();
        ...
        extern void snake_spawn(); // This tells the shell the function exists
        snake_spawn();             // This calls the function from snake_app.ino
    }
...
void input_task(void* arg) {
    ...
    // If we are in-app, the app polls for its own input.
    // This task does nothing else.
    if (current_ui_mode == UI_APPLICATION) {
        task_sleep(20);
        return;
    }
...
void setup() {
    ...
    task_create("cpumon", NULL, NULL, 1,
                ...
                "CPU usage monitor");#include <Adafruit_GFX.h>
...
while (file) {
    ...
    task_create("tempmon", NULL, NULL, 1,
                TASK_TYPE_SERVICE, TASK_FLAG_PROTECTED | TASK_FLAG_RESPAWN, 0,#include <Adafruit_GFX.h>
...
while (file) {
    ...
    klog(0, "KERNEL: Boot complete v9.6.1 (Base)");
