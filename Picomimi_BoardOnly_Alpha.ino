//Picomimi v8.5 Alpha, testing pending, code review pending
...
public:
    virtual size_t write(uint8_t c) {
        ...
        if (c == '\n') {
            ...
            view_offset = 0;
            term_dirty = true; // Signal a redraw is needed
        } else if (c != '\r') {
...
void keyboard_render_full() {
    ...
    drawKeyboard();
    lastRow = -1; // Force redraw of highlight
    lastCol = -1;
    updateKeyHighlight();
}
...
void keyboard_handle_input() {
    ...
    // Start button - move down (change row)
...
void vfs_list() {
    ...
    kout.println("\n=== VFS Contents ===");
    kout.println("ID  Name             Type   Size    Owner");
    kout.println("--  ---------------  -----  ------  -----");
...
void fs_init() {
    ...
    kout.println("[FS] SD card detected, increasing speed...");
    SD.end();
    delay(100);
    
    if (!SD.begin(SD_CS, 4000000)) {
        kout.println("[FS] Failed to set 4MHz speed");
...
// term_render only draws lines that have changed
void term_render() {
...
void display_task(void* arg) {
    ...
    if (ui_mode_changed) {
        ...
        if (current_ui_mode == UI_TERMINAL) {
            memset(screen_buffer, 1, sizeof(screen_buffer)); // Invalidate buffer
            term_dirty = true;
        } else { // UI_KEYBOARD
    ...
    // In keyboard mode, drawing is handled directly by the input task for responsiveness

    task_sleep(33); // Check for dirty screen ~30 times/sec
}
...
void shell_task(void* arg) {
    ...
    while (Serial.available()) {
        ...
        if (c == '\r' || c == '\n') {
            strncat(term_history[history_head], cmd_buffer, TERM_COLS - history_col);
            kout.println(); // This advances history and sets dirty flag
            ...
            shell_prompt();
            term_dirty = true; // Make sure the new prompt is rendered
...
void snake_init() {
    ...
    if (kernel.display_alive) {
        // Clear screen logic needed here
    }
...
void clock_task(void* arg) {
    ...
    // This app would need a dedicated screen drawing mode, incompatible with the terminal
...
void setup() {
    ...
    kout.println("========================================");
    kout.println("  RP2040 Kernel v8.5 - Keyboard UI");
    kout.println("  Full UNIX-like Operating System");
    kout.println("========================================");
    ...
    task_create("idle", idle_task, NULL, 0,
                ...
                "Kernel idle task");
    kout.println("[OK] Idle (KERNEL - CRITICAL!)");
    ...
    kout.println("\n=== Task Architecture ===");
    kout.println("KERNEL:   Core system - IMMORTAL");
    kout.println("DRIVER:   Hardware - IMMORTAL");
    kout.println("SERVICE:  System services - IMMORTAL");
    kout.println("MODULE:   Extensions - OOM PROTECTED");
    kout.println("APP:      User programs - OOM KILLABLE");
    
    kout.println("\n=== Storage Systems ===");
    kout.println("VFS: RAM-based temporary filesystem");
    kout.println("     Use 'vfscreate' to activate");
    kout.println("     Use 'vfsdedicate' to save to SD");
    if (kernel.fs_available) {
        kout.println("FS:  SD card persistent storage");
        kout.println("     Ready for use");
    } else {
        kout.println("FS:  SD card not detected");
    }
    
    kout.println("\n=== Safe Shutdown ===");
    kout.println("Use 'shutdown' command for safe poweroff");
    kout.println("- Checks for unsaved VFS files");
    kout.println("- Prompts to commit to SD");
    kout.println("- Verifies FS integrity");
    kout.println("- Closes all open files");
