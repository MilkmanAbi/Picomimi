/*
 * ╔═══════════════════════════════════════════════════════════════════════════╗
 * ║  PICOMIMI-AXISOS v14-Quiet-Otter   // Prototype Pre-release               ║
 * ╠═══════════════════════════════════════════════════════════════════════════╣
 * ║  TARGET: RP2350 (Cortex-M33 @ 250MHz nominal, 300MHz turbo)               ║
 * ║  RAM: 520KB SRAM (kernel uses ~30%, 70% for apps)                         ║
 * ║  FEATURES: FPU, DSP, TrustZone, MPU, Hardware FP                          ║
 * ╚═══════════════════════════════════════════════════════════════════════════╝
 * 
 * ============================================================================
 * PICOMIMI-AXISOS SDK - RP2350 OPTIMIZED APPLICATION DEVELOPMENT GUIDE
 * ============================================================================
 * 
 ...
 *   - CPU_PROFILE_POWERSAVE: 125MHz (low power)
 *   - CPU_PROFILE_BALANCED:  200MHz (default idle)
 *   - CPU_PROFILE_PERFORMANCE: 250MHz (nominal)
 *   - CPU_PROFILE_TURBO:     300MHz (big tasks, auto-boost)
 ...
 * 
 * ============================================================================
 * Based on Picomimi MicroOS v13.2 - Fork by uh... me? Fork for AxisOS
...
// ============================================================================
// CPU FREQUENCY GOVERNOR PROFILES
// ============================================================================

enum CPUProfile : uint8_t {
  CPU_PROFILE_POWERSAVE = 0,    // 125MHz @ 1.00V - Idle/sleep
  CPU_PROFILE_BALANCED = 1,     // 200MHz @ 1.05V - Light tasks
  CPU_PROFILE_PERFORMANCE = 2,  // 250MHz @ 1.10V - Normal operation (NOMINAL)
  CPU_PROFILE_TURBO = 3         // 300MHz @ 1.25V - Big tasks (AUTO-BOOST)
};

// Frequency table (KHz)
static const uint32_t CPU_FREQ_TABLE[] = {
  125000,  // POWERSAVE
  200000,  // BALANCED
...
// Voltage table (millivolts)
static const uint32_t CPU_VOLTAGE_TABLE[] = {
  1000,  // POWERSAVE
  1050,  // BALANCED
  1100,  // PERFORMANCE
  1250   // TURBO (safe overclock voltage)
};

// Governor state
static volatile CPUProfile current_cpu_profile = CPU_PROFILE_PERFORMANCE;
static volatile CPUProfile requested_cpu_profile = CPU_PROFILE_PERFORMANCE;
static volatile uint32_t current_freq_khz = 250000;
...
#define GOVERNOR_CHECK_INTERVAL_MS   100
#define TURBO_MAX_DURATION_MS        10000   // Max 10s turbo
#define TURBO_COOLDOWN_MS            5000
#define THERMAL_THROTTLE_TEMP        70.0f   // Celsius

// Hysteresis thresholds: scale-up at higher load, scale-down at lower load
// This prevents "thrashing" between frequency states
#define TURBO_SCALE_UP_THRESHOLD     85      // Boost to TURBO when load >= 85%
#define TURBO_SCALE_DOWN_THRESHOLD   75      // Drop from TURBO when load < 75%
#define PERF_SCALE_UP_THRESHOLD      55      // Boost to PERFORMANCE when load >= 55%
#define PERF_SCALE_DOWN_THRESHOLD    40      // Drop from PERFORMANCE when load < 40%
#define BALANCED_SCALE_UP_THRESHOLD  25      // Boost to BALANCED when load >= 25%
#define BALANCED_SCALE_DOWN_THRESHOLD 15     // Drop from BALANCED when load < 15%

// Input boost configuration
#define INPUT_BOOST_PROFILE          CPU_PROFILE_PERFORMANCE  // Profile on input
#define INPUT_BOOST_DURATION_MS      500     // How long to maintain boost after input
...
struct TCB {
  ...
  uint32_t stack_high_water;    // New: stack usage tracking
  ...
  uint8_t cpu_profile_hint;     // New: CPU profile hint for governor
  uint16_t respawn_count;
  uint16_t page_faults;
  uint16_t context_switches;
  bool mem_blocked;
  bool is_cpu_abuser;
  bool is_rt_task;              // New: real-time task flag
  uint8_t _pad[1];
} CACHE_ALIGNED;
...
// ============================================================================
// RP2350 CPU FREQUENCY GOVERNOR - FORWARD DECLARATIONS
// ============================================================================
...
void governor_input_boost();              // Input boost hook for UI responsiveness
...
// ============================================================================
// RP2350 CPU FREQUENCY GOVERNOR IMPLEMENTATION
// ============================================================================

// Apply a CPU profile (internal helper)
static void governor_apply_profile(CPUProfile profile) {
  ...
  uint32_t target_freq = CPU_FREQ_TABLE[profile];
  uint32_t target_voltage_mv = CPU_VOLTAGE_TABLE[profile];
  
  // Convert millivolts to vreg voltage enum
  enum vreg_voltage vreg_volt;
  if (target_voltage_mv <= 850) {
    vreg_volt = VREG_VOLTAGE_0_85;
  } else if (target_voltage_mv <= 900) {
    vreg_volt = VREG_VOLTAGE_0_90;
  } else if (target_voltage_mv <= 950) {
    vreg_volt = VREG_VOLTAGE_0_95;
  } else if (target_voltage_mv <= 1000) {
    vreg_volt = VREG_VOLTAGE_1_00;
  } else if (target_voltage_mv <= 1050) {
    vreg_volt = VREG_VOLTAGE_1_05;
  } else if (target_voltage_mv <= 1100) {
    vreg_volt = VREG_VOLTAGE_1_10;
  } else if (target_voltage_mv <= 1150) {
    vreg_volt = VREG_VOLTAGE_1_15;
  } else if (target_voltage_mv <= 1200) {
    vreg_volt = VREG_VOLTAGE_1_20;
  } else if (target_voltage_mv <= 1250) {
    vreg_volt = VREG_VOLTAGE_1_25;
  } else {
    vreg_volt = VREG_VOLTAGE_1_30;
  }
  
  // When increasing frequency, raise voltage first
  if (target_freq > kernel.governor.current_freq_khz) {
    vreg_set_voltage(vreg_volt);
    busy_wait_us(100);  // Wait for voltage to stabilize
  }
  
  // Set the clock frequency
  if (!set_sys_clock_khz(target_freq, true)) {
    // If failed, try a fallback
    klog(2, "GOV: Clock set failed, using fallback");
    set_sys_clock_khz(250000, true);  // Fallback to nominal
    kernel.governor.current_freq_khz = 250000;
    kernel.governor.current_profile = CPU_PROFILE_PERFORMANCE;
    return;
  }
  
  // When decreasing frequency, lower voltage after
  if (target_freq < kernel.governor.current_freq_khz) {
    vreg_set_voltage(vreg_volt);
  }
  ...
  char buf[64];
  const char* profile_names[] = {"POWERSAVE", "BALANCED", "PERFORMANCE", "TURBO"};
  snprintf(buf, sizeof(buf), "GOV: %s @ %lu KHz", 
           profile_names[profile], target_freq);
  klog(0, buf);
}
...
void governor_init() {
  ...
  // Just set initial state - DON'T change clock frequency at boot!
  // The Arduino core has already configured the clock, changing it
  // during early init can crash the RP2350
  kernel.governor.current_profile = CPU_PROFILE_PERFORMANCE;
  kernel.governor.requested_profile = CPU_PROFILE_PERFORMANCE;
  kernel.governor.current_freq_khz = 250000;  // Assume nominal
  kernel.governor.target_freq_khz = 250000;
  kernel.governor.turbo_active = false;
  kernel.governor.thermal_throttled = false;
  kernel.governor.user_override = false;
  kernel.governor.last_change_ms = get_time_ms();  // Set this so tick doesn't immediately fire
  kernel.turbo_enabled = true;
  
  // NOTE: We don't call governor_apply_profile here!
  // Let the system stabilize first, governor_tick will handle scaling later
  
  kout.println("[GOV] CPU Governor initialized (250MHz nominal)");
  kout.println("[GOV] Profiles: POWERSAVE(125) BALANCED(200) PERF(250) TURBO(300)");
}

// Periodic governor tick - called from scheduler
void governor_tick() {
  uint32_t now = get_time_ms();
  
  // Don't check too often
  if (now - kernel.governor.last_change_ms < GOVERNOR_CHECK_INTERVAL_MS) {
    return;
  }
  
  // Check thermal limits first
  governor_thermal_check();
  
  // If thermally throttled, don't increase frequency
  ...
  // If user override is set, respect it
  ...
  // Update load history (rolling average)
  ...
  // Determine target profile using HYSTERESIS (prevents thrashing)
  CPUProfile current = kernel.governor.current_profile;
  CPUProfile target = current;  // Default: stay at current
  
  // Check scale-up conditions (higher thresholds)
  if (kernel.governor.avg_load >= TURBO_SCALE_UP_THRESHOLD && kernel.turbo_enabled) {
    target = CPU_PROFILE_TURBO;
  } else if (kernel.governor.avg_load >= PERF_SCALE_UP_THRESHOLD && current < CPU_PROFILE_PERFORMANCE) {
    target = CPU_PROFILE_PERFORMANCE;
  } else if (kernel.governor.avg_load >= BALANCED_SCALE_UP_THRESHOLD && current < CPU_PROFILE_BALANCED) {
    target = CPU_PROFILE_BALANCED;
  }
  // Check scale-down conditions (lower thresholds)
  else if (current == CPU_PROFILE_TURBO && kernel.governor.avg_load < TURBO_SCALE_DOWN_THRESHOLD) {
    target = CPU_PROFILE_PERFORMANCE;
  } else if (current == CPU_PROFILE_PERFORMANCE && kernel.governor.avg_load < PERF_SCALE_DOWN_THRESHOLD) {
    target = CPU_PROFILE_BALANCED;
  } else if (current == CPU_PROFILE_BALANCED && kernel.governor.avg_load < BALANCED_SCALE_DOWN_THRESHOLD) {
    target = CPU_PROFILE_POWERSAVE;
  }
  
  // Handle turbo time limit
  if (target == CPU_PROFILE_TURBO) {
    if (!kernel.governor.turbo_active) {
      kernel.governor.turbo_start_ms = now;
      kernel.governor.turbo_active = true;
    } else {
      // Check turbo duration limit
      uint32_t turbo_duration = now - kernel.governor.turbo_start_ms;
      if (turbo_duration > TURBO_MAX_DURATION_MS) {
        target = CPU_PROFILE_PERFORMANCE;
        kernel.governor.turbo_active = false;
        klog(1, "GOV: Turbo timeout, cooling down");
      }
    }
  } else {
    kernel.governor.turbo_active = false;
...
// Set CPU profile manually
void governor_set_profile(CPUProfile profile) {
  if (profile > CPU_PROFILE_TURBO) return;
...
void governor_request_turbo(uint32_t duration_ms) {
  ...
  if (duration_ms > TURBO_MAX_DURATION_MS) {
    duration_ms = TURBO_MAX_DURATION_MS;
  }
  
  kernel.governor.turbo_start_ms = get_time_ms();
  kernel.governor.turbo_active = true;
  kernel.governor.user_override = false;  // Allow auto-management after
  
  governor_apply_profile(CPU_PROFILE_TURBO);
  
  klog(0, "GOV: Turbo boost requested");
}
...
void governor_thermal_check() {
  ...
  if (temp >= THERMAL_THROTTLE_TEMP) {
    if (!kernel.governor.thermal_throttled) {
      ...
      klog(2, "GOV: THERMAL THROTTLE ACTIVATED");
      
      // Immediately drop to balanced
      governor_apply_profile(CPU_PROFILE_BALANCED);
    }
...
void __attribute__((hot)) task_yield() {
  ...
  uint32_t task_count = kernel.task_count;  // Cache for faster access
  uint64_t now = get_time_ms();
...
void fs_log_init() {
  ...
  if (!logFile) {
    ...
    if (logFile) {
      logFile.println("=== Picomimi RTOS v14.0 System Log ===");
...
bool fs_mount() {
  ...
  pmfs.log_system("Picomimi v14.0 kernel boot");
...
void idle_task(void* arg) {
  if (kernel.governor.current_profile >= CPU_PROFILE_BALANCED) {
    // WFI: Wait For Interrupt - low power until next interrupt
...
void shell_execute(char* cmd) {
  ...
  // CPU Governor commands
    ...
    const char* profile_names[] = {"POWERSAVE", "BALANCED", "PERFORMANCE", "TURBO"};
    kout.print("Profile: ");
    kout.println(profile_names[kernel.governor.current_profile]);
    kout.print("Frequency: ");
    ...
    kout.println("%");
    kout.print("Turbo: ");
    kout.println(kernel.turbo_enabled ? (kernel.governor.turbo_active ? "ACTIVE" : "enabled") : "disabled");
    kout.print("Thermal: ");
    kout.println(kernel.governor.thermal_throttled ? "THROTTLED" : "OK");
    kout.print("Input Boost: ");
    kout.println(kernel.governor.input_boost_active ? "ACTIVE" : "idle");
    kout.print("Override: ");
    ...
    kout.println("[GOV] Set to POWERSAVE (125MHz)");
  }
  else if (strcmp(cmd, "gov balanced") == 0) {
    governor_set_profile(CPU_PROFILE_BALANCED);
    kout.println("[GOV] Set to BALANCED (200MHz)");
  }
    ...
    if (!kernel.turbo_enabled) {
      kout.println("[GOV] Turbo is disabled");
      ...
      kout.println("[GOV] Set to TURBO (300MHz) - manual override");
    }
  }
  else if (strcmp(cmd, "gov turbo on") == 0) {
    kernel.turbo_enabled = true;
    kout.println("[GOV] Turbo boost ENABLED");
  }
...
void cmd_help() {
  kout.println("\n=== Picomimi-AxisOS MicroOS v14.0 Commands ===");
  kout.println("\n--- System ---");
...
public:
  ...
  // Hint that a heavy task is starting (may trigger turbo)
  static void HintHeavyTaskStart() {
    governor_request_turbo(TURBO_MAX_DURATION_MS);
  }
...
void setup() {
  ...
  kout.println("╔═══════════════════════════════════════════════════════════╗");
  kout.println("║  PICOMIMI-AXISOS FORK v14-Quiet-Otter                     ║");
  kout.println("║  \"Modified\" Edition for RP2350                          ║");
  kout.println("╠═══════════════════════════════════════════════════════════╣");
  ...
  kout.print("Governor: ");
  const char* profile_names[] = {"POWERSAVE", "BALANCED", "PERFORMANCE", "TURBO"};
  kout.print(profile_names[governor_get_profile()]);
  kout.print(" (Turbo: ");
  ...
  klog(0, "KERNEL: AxisOS v14-Quiet-Otter boot");
